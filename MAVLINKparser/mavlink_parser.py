...
def parse(mavObj,msg, debug):    
    ...
    if MAVmsg != None:
        ...
        if MAVmsg.get_header().msgId == mavlink0.MAVLINK_MSG_ID_BAD_DATA:
            ...
            if DEBUG == 1:
                # lab1
                return mavlink0.MAVLINK_MSG_ID_HEARTBEAT, randint(0,8), randint(0,8), MAVmsg.get_payload()
                #lab<1
        ...
        # if ping message received, response with correct ping
            ...
            return mavlink0.MAVLINK_MSG_ID_MISSION_ACK, MAVmsg.get_header().srcSystem, MAVmsg.get_header().srcComponent, MAVmsg.get_payload()
...
###############################################################################
## handle the reception of a parameter value request
###############################################################################
...
def handle_mission_item_send(msg, obj): 
    ...
    if DEBUG == 0:
        return mavlink0.MAVLINK_MSG_ID_MISSION_ITEM, sysID, compID, msg.get_payload()
    else:
        qc.generate(mavlink0.MAVLINK_MSG_ID_MISSION_ITEM, sysID, compID, msg.get_payload(), obj)
        
    
###############################################################################
## handles received mode_set message
###############################################################################        
